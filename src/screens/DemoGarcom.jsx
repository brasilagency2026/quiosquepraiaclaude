  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F0F7FF', fontFamily: 'Inter,sans-serif' }}>
      ...
      <div style={{ background: '#0D2137', padding: '16px 20px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontFamily: "'Baloo 2',cursive", fontSize: 20, fontWeight: 800, color: '#F5E6C8' }}>
            🛵 Entregas
            {totalPendente > 0 && <span style={{ background: '#FF6B6B', borderRadius: '50%', width: 22, height: 22, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, color: 'white', marginLeft: 8 }}>{totalPendente}</span>}
          </div>
          <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>{session.nom} · Demo · {entregues.length} entregue(s)</div>
        </div>
      ...
      <div style={{ background: '#FFF9E6', border: '1px solid #FDE68A', margin: '12px 16px 0', borderRadius: 10, padding: '8px 12px', fontSize: 12, color: '#92400E' }}>
        💡 Demo — clique nos botões para simular o fluxo de entrega
      </div>
      ...
      <div style={{ padding: '16px 16px 60px' }}>
          ...
          <>
            ...
            {pedidosDinheiro.map(pedido => (
              <div key={pedido.id} style={{ background: 'white', borderRadius: 16, overflow: 'hidden', marginBottom: 12, boxShadow: '0 2px 12px rgba(0,0,0,0.08)', border: '2px solid #F59E0B' }}>
                <div style={{ background: '#F59E0B', padding: '10px 16px', display: 'flex', justifyContent: 'space-between' }}>
                ...
                <div style={{ padding: '14px 16px' }}>
                  ...
                  <button onClick={() => { atualizar(pedido.id, 'cozinha'); showToast(`✅ Dinheiro recebido! Pedido #${pedido.numero} enviado à cozinha.`) }}
                    style={{ width: '100%', background: '#F59E0B', color: '#0D2137', border: 'none', borderRadius: 10, padding: 14, fontFamily: "'Baloo 2',cursive", fontSize: 16, fontWeight: 700, cursor: 'pointer', marginTop: 12 }}>
          ...
          <>
            ...
            {pedidosProntos.map(pedido => (
              <div key={pedido.id} style={{ background: 'white', borderRadius: 16, overflow: 'hidden', marginBottom: 12, boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
                <div style={{ background: '#06D6A0', padding: '10px 16px', display: 'flex', justifyContent: 'space-between' }}>
                  <h4 style={{ fontFamily: "'Baloo 2',cursive", fontSize: 18, fontWeight: 800, color: '#0D2137' }}>Pedido #{pedido.numero}</h4>
                  <span style={{ fontFamily: "'Baloo 2',cursive", fontSize: 15, fontWeight: 800, color: '#0D2137' }}>🏖️ {pedido.parasolNumero}</span>
                </div>
                <div style={{ padding: '14px 16px' }}>
                  ...
                  <button onClick={() => { atualizar(pedido.id, 'entregue'); showToast(`🏖️ Pedido #${pedido.numero} entregue!`) }}
                    style={{ width: '100%', background: '#0D2137', color: 'white', border: 'none', borderRadius: 10, padding: 12, fontFamily: "'Baloo 2',cursive", fontSize: 16, fontWeight: 700, cursor: 'pointer', marginTop: 12 }}>
        ...
        {totalPendente === 0 && (
          <div style={{ textAlign: 'center', padding: '60px 0', color: '#94A3B8' }}>
            ...
            {entregues.length > 0 && <p style={{ fontSize: 14, marginTop: 8, color: '#06D6A0' }}>{entregues.length} pedido(s) entregue(s) 🎉</p>}
          </div>
        )}

        {/* Histórico */}
        {entregues.length > 0 && (
          <div style={{ marginTop: 24, background: 'white', borderRadius: 14, padding: '14px 16px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', opacity: 0.7 }}>
            <p style={{ fontFamily: "'Baloo 2',cursive", fontSize: 14, fontWeight: 700, color: '#64748B', marginBottom: 8 }}>📋 Entregues hoje</p>
            {entregues.map(p => (
